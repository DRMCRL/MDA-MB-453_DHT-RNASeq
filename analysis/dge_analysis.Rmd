...
samples <- config$samples %>%
  ...
  mutate(name = str_extract(desc, "MDA[0-9].++")) %>%
  dplyr::select(-desc) %>%
  mutate_if(
...
However, sample MDA6 was below the common-use minimum library size in both treatment groups and whilst discarding this sample is not advised, this should be noted for all downstream analysis.
Similarly, the DHT-treated sample for MDA2 showed a library size which was nearly double the rest of the samples
...
Despite concerns raised by the library sizes, sample MDA6 appears to be relatively consistent with other samples of the same treatment group.
However, MDA2 showed significant divergence from the others in both treatment groups, whilst the vehicle-treated (i.e. control) sample from MDA5 was also a notable outlier.
...
```{r plotPcaPost, fig.height=8, fig.width=8, fig.cap="*PCA on logCPM after performing CQN. Much of the variability previously seen in with replicate 5 has now been minimised, whilst replicate 2 still appears to be a considerable outlier. However, PC1 and PC2 now capture the biology in question, with vectors denoting the shift from vehicle to treatment shown as arrows shown in panel C. Panel B does highlight that along PC2 the control sample from replicate 2 does more closely resemble the treated samples from all other replicates.*"}
a <- pcaPost %>%
  autoplot(
    data = dge$samples, 
    x = 1, y = 3,
...
pdf(
  here::here("docs/assets/pcaPost.pdf"), width = 8, height = 8
)
c
dev.off()
```
...
topTable %>%
  ...
  geom_text_repel(
    aes(label = gene_name, colour = DE),
    data = . %>%
      dplyr::filter(abs(logFC) > 2 | PValue < 5e-10),
    show.legend = FALSE
  ) +
...
## Exclusion of Replicate 2

Given the concerns raised by the above MA plot and the checks for bias, the above analysis was repeated, excluding replicate 2 (not shown).
Whilst results were broadly comparable, this did not improve the bias observed in the low-expressed genes.
Instead, this bias was attributed to genes expressed at low levels in only one of the treatment groups.

Any enrichment analysis should incorporate a bias offset, such as offered by the package `goseq`.
Enrichment analysis using ranked lists should be undertaken with caution.


# Compare DEGs to hub genes detected in the PDE study


```{r}
PDE_DHT_hubGene <- readRDS("~/WenjunLiu_Thesis_Chapter4/output/PDE_DHT_hubGene.rds")
lobular_diff <- PDE_DHT_hubGene %>%
  dplyr::filter(!Align) 
lobular_overlap <- topTable %>%
  dplyr::filter(DE) %>%
  dplyr::select(
    gene_name, logFC, 
  ) %>%
...
`r nrow(PDE_DHT_hubGene)` hub genes potentially driving the perturbation of pathways in DHT-treated IDC-like PDEs were loaded and compared against DEGs detected in DHT-treated MDA-MB-453 cells. Only 4 overlapping genes were found. All of them had directional opposing median ssFCs between IDC- and ILC-like PDEs, but the direction of changes observed in MDA-MB-453 cells aligned more to the DHT-treated IDC-like PDEs than ILC-like PDEs. Only the HIF1A gene, which was upregulated in MDA-MB-453 cells, had directional concordant chagne between the cell line and ILC-like PDEs and opposite change to IDC-like PDEs.

```{r}
temp <- lobular_overlap %>%
  dplyr::select(
    Gene = gene_name, 
    `MDA-MB-453 logFC` = logFC, 
    `ILC-like PDE logFC` = `ILC-like`, 
    `IDC-like PDE logFC` = `IDC-like`
  ) %>%
  mutate_if(is.numeric, round, 4) 
temp %>%
  datatable() %>%
  formatStyle(
    str_subset(colnames(temp), "logFC"), 
...
These above results were exported as the file `MDA-MB-453_RNASeq.tsv`.
...
topTable %>%
  ...
  write_tsv(
    file = here::here("output/MDA-MB-453_RNASeq.tsv")
  )
```
...
2^logCPM %>%
  ...
  write_tsv("output/MDA-MB-453_CPM.tsv")
```
